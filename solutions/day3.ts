...
function solve(input: string[]): number[] {
    ...
    let distances = intersections.map((i) =>
        calcManhattanDistance({ x: 0, y: 0 }, i)
    );
    let distance2 = intersections.map((i) => calcTotalWireDistance(i, a, b));
    return [
        Math.min(...distances.slice(1, distances.length)),
        Math.min(...distance2.slice(1, distance2.length)),
    ];
}
